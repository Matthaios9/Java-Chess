...
public class GUIClient extends JPanel {
    ...
    public GUIClient(Board b, White w, Black bl, JPanel txt) {
        try{
            // Creating a new socket on given port
            socket = new Socket("localhost",8888);
            // Getting the input and output streams of socket
            output = new ObjectOutputStream(socket.getOutputStream());
            input = new ObjectInputStream(socket.getInputStream());
            // Setting turn initially as false
            turn = false;
        }
        // Exception Handling
        ...
        // Creating timer to call host_input after 2 seconds
        Timer first_timer = new Timer(2000, new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                host_input();
            }
        });

        // Setting the repeats off for timer and starting the timer
        first_timer.setRepeats(false);
        first_timer.start();
    ...
    private class MoveHandler extends MouseAdapter {
        ...
        public void mousePressed(MouseEvent event) {
            ...
            if (turn) {
                ...
                if (highlighted.size() > 0) {
                    ...
                    for (PiecePanel p : highlighted) {
                        ...
                        if (p == currently_selected) {
                             ...
                             */
                            move = String.format("%d %d %d %d", previously_selected.x_pos, previously_selected.y_pos, x,
                                    y);
                            String s = blackPlayer.move(previously_selected.x_pos, previously_selected.y_pos, x, y);
                            ...
                            if ((temp_piece instanceof Pawn) && (temp_piece.getX() == 7 || temp_piece.getX() == 0)) {
                                ...
                                // Sending the move to host socket
                                try {
                                    output.writeObject(move + " Queen");
                                } catch (Exception se) {
                                    se.printStackTrace();
                                }
                ...
                // current turn
                if (piece_board.getPiece(x, y) != null && piece_board.getPiece(x, y).color == "BLACK") {
    ...
    // Method to get input from host socket
    private void host_input() {
        ...
        try {
            // Reading move from host
            String move = (String) input.readObject();
            // Getting move coordinates from input by host socket
